...
export const WORKFLOW_TOOL_DESCRIPTION = [
  ...
  "Available globals: agent(prompt, opts), parallel(thunks), pipeline(items, ...stages), phase(title), log(message), workflow(nameOrRef, args), args, cwd.",
].join(" ");
...
export const WORKFLOW_GUIDELINES: string[] = [
  ...
  "For the workflow tool, workflow(nameOrRef, args) runs a saved workflow (by name) or a scriptPath inline as a sub-step, sharing this run's concurrency cap and agent counter. Nesting is one level only.",
