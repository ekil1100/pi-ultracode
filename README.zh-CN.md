...
Ultracode 不会修改父会话的 effort。父代理按**分配给每个子代理的具体任务**通过模型后缀选择最合适的受支持档位，不固定默认 `medium` 或 `high`，也不盲目追求最低或最高档。
...
这些分档是任务选择标准，不是跨提供商统一的能力刻度。Workflow UI 展示模型裁剪后实际生效的 effort；未指定后缀时，子会话使用正常的用户/模型配置。系统提示词与 workflow 指南共用 `src/effort-policy.ts`，不引入自动修改父会话 effort 的路由器。skeptic 和独立 synthesis agent 都不是默认步骤。
